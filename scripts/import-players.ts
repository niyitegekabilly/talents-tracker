...
interface CSVPlayer {
  ...
  'Niveau': string;
  "Niveau d'âge": string;
  "Période de l'enregistrement": string;
  "Date d'enregistrement ajoutée": string;
  "Date d'inscription approuvée": string;
  'Téléphone à la maison': string;
  "Numéro d'identification": string;
  'Nationalité principale': string;
  'Ville de naissance': string;
  'Pays de naissance': string;
  'Sexe': string;
  'Date depuis': string;
  "Date jusqu'à": string;
}
...
async function importPlayers() {
  const csvFilePath = path.resolve(__dirname, '../FULL DETAILS PLAYERS 2024-25.csv');
  const fileContent = fs.readFileSync(csvFilePath, { encoding: 'utf-8' });

  parse(fileContent, {
    columns: true,
    skip_empty_lines: true,
  }, async (error: Error | null, records: CSVPlayer[]) => {
    if (error) {
